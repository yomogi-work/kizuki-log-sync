...

{
  ...
  "mentoring_seeds": [
    "『日誌内の学生の言葉を引用』→ その体験を踏まえた観察の視点①（※前回からの継続フラグがある場合は、必ずその文脈を引き継ぎ、一歩深めた視点を第一候補にすること）",
...
- 必ず日誌内の学生の言葉を『』で引用することから始める。
- 「明日〜してみてください」ではなく、「明日〜に気づいたら、それを覚えておいてください」という形にする。
- 一般的なコミュニケーション論（オープンエンド型質問、傾聴スキル等）を直接アドバイスしてはいけない。

### Seedの良い例
『短い返答で会話が終わってしまう』と感じた瞬間、自分がどんな質問をした直後だったかを明日一度だけ振り返ってみてください。

### Seedの悪い例（絶対に出力しないこと）
「オープンエンド型の質問を用いることで、患者のニーズを引き出すことが可能です」
→ これは教科書的な行動指示であり、この学生のこの日誌から生まれた言葉が一つもない。

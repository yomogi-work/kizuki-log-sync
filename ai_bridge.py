...
### 手順1: 日誌の全文を読む。
※絶対のルール※
抽出の対象とするのは「学生ログ①」および「学生ログ②」の中に書かれた学生自身の言葉だけです。
「前回の指導ポイント」や「指導者メモ」の内容から Step0 (evidence) のテキストを抜き出すことは絶対に禁止です。
...
def analyze_journal(week: int, log_achieved: str, log_unachieved: str,
    ...
    user_prompt = build_user_prompt(
        week, log_achieved, log_unachieved, instructor_notes
    )
...
3. 【文脈の接続（無理な接ぎ木は厳禁）】
   - 今日のStep0が「薬理作用や業務等の知識」であり、過去のサマリーが「対話等のソフトスキル」である場合など、トピックの次元が違う場合は、絶対に２つを関連付けないでください。
   - トピックが違う場合は、過去のサマリーは完全に無視し、今日のStep0の引用だけを起点にコメントを作ること。
   - 過去のサマリーと今日のStep0に「明確な連続性」がある場合のみ、「ずっと意識してきた〇〇ですが〜」のように連続性に触れること。
